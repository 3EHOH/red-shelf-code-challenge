...
public class QuickMemberSelectCHC {
	...
	private boolean accumulateMember (String s) {
		...
		if (sMemberEID.isEmpty()) {
			...
			if (isInList(sMemberEID)) {
				...
				if (sMemberEID.equals("528567865") ) {
	...
	private boolean selectClaim (String s) {
		...
		// column 6 is the member_id
		String sMemberEID = in[6].trim();
		...
		if(bResult)
			providerAccumulator.add(in[12].trim());
	...
	private boolean selectProvider (String s) {
		...
		if (providerAccumulator.contains(in[0].trim()))
			bResult = true;
	...
	public static void main(String[] args) throws IOException {
		...
		QuickMemberSelectCHC instance = new QuickMemberSelectCHC();
	...
	private static org.apache.log4j.Logger log = Logger.getLogger(QuickMemberSelectCHC.class);
