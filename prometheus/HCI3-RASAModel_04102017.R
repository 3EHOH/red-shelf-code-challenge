...
episode_ref <- as.character(myfunc(con, paste0("SELECT DISTINCT epi_number FROM ", paste(db, "ra_episode_data", sep = ".")))[,1])
...
if (!per_episode) {
  ra_episode_data <- dbReadTable(con, "epbuilder.ra_episode_data")
  ra_cost_use <- dbReadTable(con, "epbuilder.ra_cost_use")
  ra_subtypes <- dbReadTable(con, "epbuilder.ra_subtypes")
  ra_riskfactors <- dbReadTable(con, "epbuilder.ra_riskfactors")
...
epi.models <- lapply(data.dictionary$EPISODE_ID, function(epi) {
  ...
  if (per_episode) {
    ra_episode_data <- myfunc(con, paste0("SELECT * FROM ", paste(db, "ra_episode_data", sep = "."), " WHERE epi_number = '", epi, "'"))
    ra_cost_use <- myfunc(con, paste0("SELECT * FROM ", paste(db, "ra_cost_use", sep = "."), " WHERE epi_number = '", epi, "'"))
    ra_subtypes <- myfunc(con, paste0("SELECT * FROM ", paste(db, "ra_subtypes", sep = "."), " WHERE epi_number = '", epi, "'"))
    ra_riskfactors <- myfunc(con, paste0("SELECT * FROM ", paste(db, "ra_riskfactors", sep = "."), " WHERE epi_number = '", epi, "'"))
  ...
  if (!inherits(per_sum, "try-error") && nrow(per_sum) > 0) {
    ...
    #    diag.return <- cbind(type = choice, rbind(use.model.diag, cost.model.diag))
        ...
        return.list <- list(use = clean_coef(use.typ.model), cost = my.coefs, expected = my.list, #diags = diag.return, 
        model_diags = model.diag, obs_diags = obs.diag, use_diags = use.diag)
        return.list
      } else {
...
if (output == "db") {
  ...
  dbWriteTable(con, name = "ra_exp_cost", value = final_expected,overwrite=TRUE)
  ...
  write.csv(final_expected, file = "ra_exp_cost.csv")
