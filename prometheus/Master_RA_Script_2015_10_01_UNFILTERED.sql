...
insert into tmp_trig_claims 
select
e.master_episode_id,
case when max(cc.allowed_amt) then cc.master_claim_id end as master_claim_id,
e.episode_id,
e.episode_type,
e.trig_begin_date,
max(e.trig_end_date) as trig_end_date,
...
AND me.annualized=0
AND me.filter_id=0
AND e.episode_type IN ('A','P') and t.claim_line_type_code='IP';
...
AND me.annualized=0
AND me.filter_id=0
AND e.episode_type IN ('P') and t.claim_line_type_code='IP';
...
AND me.annualized=0
AND me.filter_id=0
AND e.episode_type IN ('A') and t.claim_line_type_code='IP';
...
insert into ra_cost_use select
	...
	if((me.cost_t + me.cost_t) - ifnull(t.allowed_amt,0)>0,1,0) as value
FROM master_epid_level me
left join tmp_trig_claims t on me.master_episode_id=t.master_episode_id
...
insert into ra_cost_use select
	...
	(me.cost_t + me.cost_t) - ifnull(t.allowed_amt,0) as value
FROM master_epid_level me
left join tmp_trig_claims t on me.master_episode_id=t.master_episode_id
JOIN episode e on me.master_episode_id=e.master_episode_id
Where me.claim_type='CL'
...
insert into ra_cost_use select
	...
	(me.cost_t + me.cost_t) - ifnull(t.split_allowed_amt,0) as value
FROM master_epid_level me
left join tmp_trig_claims t on me.master_episode_id=t.master_episode_id
JOIN episode e on me.master_episode_id=e.master_episode_id
Where me.claim_type='CL'
...
#WHERE filter_id=1 and (age_limit_lower + age_limit_upper + episode_cost_upper +episode_cost_lower )>=1
