...
/**
 * copied from org.apache.ibatis.jdbc;
 * @author Warren
 * 
 * I needed to fix their comment handling
 *
 */
...
public class ScriptRunner {
  ...
  private boolean commentInProgress = false; // wjm
  ...
  private void executeLineByLine(Reader reader) {
    ...
    try {
      ...
      while ((line = lineReader.readLine()) != null) {
    	  ...
    	  // wjm - try to ignore any characters within a /* */ pair
    	  // some comments were causing unhappy situations in handleLine
    	  if (commentInProgress) {
    		  log.info("Comment in progress: " + line);
    		  if (line.contains(COMMENT_END)) {
    			  //log.info(line.substring(0, line.indexOf(COMMENT_END)));
    			  commentInProgress = false;
    			  if (line.length() > line.indexOf(COMMENT_END)+2) 
    				  line = line.substring(line.indexOf(COMMENT_END)+2);
    			  else
    				  continue;
    		  }
    		  else {
    	  ...
    	  else if (line.contains(COMMENT_START)) {
    		  log.info("Comment in progress: " + line);
    		  ...
    		  if (line.contains(COMMENT_END)) {
    			  ...
    			  commentInProgress = false;
    			  if (line.length() > line.indexOf(COMMENT_END)+2)
    				  line = line + line.substring(line.indexOf(COMMENT_END)+2);
    			  else
    				  continue;
    		  }
    		  else{
    	  ...
    	  log.info("Command in progress: " + line);	  
  ...
  private boolean lineIsComment(String trimmedLine) {
    return trimmedLine.startsWith("//") || trimmedLine.startsWith("--") || trimmedLine.startsWith("#");		// wjm - needed to add pound sign
  }
  ...
  private static org.apache.log4j.Logger log = Logger.getLogger(ScriptRunner.class);
