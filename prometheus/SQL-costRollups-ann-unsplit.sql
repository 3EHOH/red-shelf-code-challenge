...
FROM episode e 
JOIN `episode_builder-5.3.004`.episode be ON e.episode_id = be.episode_id
AND be.END_OF_STUDY=1
AND be.VERSION=(SELECT MAX(VERSION) FROM `episode_builder-5.3.004`.episode)
WHERE e.episode_begin_date<
(SELECT DATE_ADD(data_end_date,INTERVAL -1 YEAR) FROM run);
...
/* grab full level 1 costs for children */
...
/* grab level 1 costs for parents */
INSERT INTO tmp_ann_mel
SELECT DISTINCT
t.master_episode_id,
1 AS level,
'P' AS parent_child_id,
if (if (SUM(allowed_amt) IS NULL, 0.0000, SUM(allowed_amt)) IS NULL, 0.0000, if (SUM(allowed_amt) IS NULL, 0.0000, SUM(allowed_amt)) ) AS cost,
if (SUM(if(sig.assigned_type='T',cc.allowed_amt,0)) IS NULL, 0.0000, SUM(if(sig.assigned_type='T',cc.allowed_amt,0)) ) AS cost_t,
if (SUM(if(sig.assigned_type='TC',cc.allowed_amt,0)) IS NULL, 0.0000, SUM(if(sig.assigned_type='TC',cc.allowed_amt,0)) ) AS cost_tc,
if (SUM(if(sig.assigned_type='C',cc.allowed_amt,0)) IS NULL, 0.0000, SUM(if(sig.assigned_type='C',cc.allowed_amt,0)) ) AS cost_c
FROM tmp_annids t
