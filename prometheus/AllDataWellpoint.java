...
public class AllDataWellpoint extends AllDataAbstract implements AllDataInterface {

	public AllDataWellpoint(HashMap<String, String> parameters) {
	...
	public List<ClaimServLine> getAllServiceClaims() {
		...
		try {
			if (cdIf == null) {
				if (errMgr == null)
					cdIf = new ClaimDataFromWellpoint();
				else
					cdIf = new ClaimDataFromWellpoint(errMgr);
				cdIf.addSourceFile(parameters.get("claim_hdr_file"));
				cdIf.addSourceFile(parameters.get("claim_dtl_file"));
			}
	...
	boolean checkClaimsParametersOK() {
		return (parameters.get("claim_hdr_file") == null)  ||  (parameters.get("claim_dtl_file") == null);
	}
	...
	public Map<String, List<Enrollment>> getAllEnrollments() {
		...
		try {
			if (enIf == null) {
				if (errMgr == null)
					enIf = new EnrollmentDataFromWellpoint();
				else
					enIf = new EnrollmentDataFromWellpoint(errMgr);
	...
	public Map<String, PlanMember> getAllPlanMembers() {
		...
		try {
			if (pmIf == null) {
				if (errMgr == null)
					pmIf = new PlanMemberDataFromWellpoint();
				else
					pmIf = new PlanMemberDataFromWellpoint(errMgr);
	...
	public List<ClaimRx> getAllRxClaims() {
		...
		try {
			if(rxIf == null) {
				if (errMgr == null)
					rxIf = new ClaimRxDataFromWellpoint();
				else
					rxIf = new ClaimRxDataFromWellpoint(errMgr);
	...
	public Map<String, Provider> getAllProviders() {
		...
		try {
			if (pvIf == null) {
				if (errMgr == null)
					pvIf = new ProviderDataFromWellpoint();
				else
					pvIf = new ProviderDataFromWellpoint(errMgr);
	...
	private static org.apache.log4j.Logger log = Logger.getLogger(AllDataWellpoint.class);
