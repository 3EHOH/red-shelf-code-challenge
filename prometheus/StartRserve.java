...
public class StartRserve {
	...
	private static final Integer [] portList = {new Integer(6311), new Integer(6312), new Integer(6313), new Integer(6314), new Integer(6315)};
	public static Integer _port;		// port being used by this JVM
	private static final Integer NO_PORT = new Integer(-1);

	
	/** simple class that start Rserve locally if it's not running already - see mainly <code>checkLocalRserve</code> method. 
	 * It spits out quite some debugging outout of the console, so feel free to modify it for your application if desired.<p>
	...
	private static boolean launchRserve(String cmd) { return launchRserve(cmd, "--no-save --slave --verbose","--no-save --slave --verbose", false); }
	...
	private static boolean launchRserve(String cmd, String rargs, String rsrvargs, boolean debug) {
		
		_port = findOpenRservePort ();
		if (_port.equals(-1)) {
			throw new IllegalStateException("Can not start Rserve.  All ports busy");
		}
			
		String _rsrvargs = rsrvargs + " --RS-port " + _port.toString();
		
		try {
			
			//System.out.println("Rserve cmd: " + cmd + "|" + rargs + "|" + rsrvargs);
			
			Process p;
			boolean isWindows = false;
			
			System.out.println("Attempting to start Rserve on port " + _port);
			String osname = System.getProperty("os.name");
			if (osname != null && osname.length() >= 7 && osname.substring(0,7).equals("Windows")) {
				isWindows = true; /* Windows startup */
				p = Runtime.getRuntime().exec("\""+cmd+"\" -e \"library(Rserve);Rserve("+(debug?"TRUE":"FALSE")+",args='"+_rsrvargs+"')\" "+rargs);
			} else /* unix startup */
				p = Runtime.getRuntime().exec(new String[] {
						"/bin/sh", "-c",
						"echo 'library(Rserve);Rserve("+(debug?"TRUE":"FALSE")+",args=\""+_rsrvargs+"\")'|"+cmd+" "+rargs
				});
		 ...
		 * We can be conservative here, because at this point the process execution itself was successful and the start up is usually asynchronous 
		 */
		
		int attempts = 5; 
		while (attempts > 0) {
			try {
				RConnection c = new RConnection("localhost", _port);
				System.out.println("Rserve has started on port " + _port);
				c.close();
				return true;
			} catch (Exception e2) {
	...
	/**
	 * find an appropriate Rserve port that isn't in use
	 * @return
	 */
	private static Integer findOpenRservePort () {
		
		Integer _returnPort = NO_PORT;
		
		for (Integer _portToCheck : portList) {
			if ( ! isRserveRunning("localhost", _portToCheck) ) {
				_returnPort = _portToCheck;
				break;
			}
		}
		
		System.out.println("findOpenRservePort returning " + _returnPort);
		return _returnPort;
	 ...
	 */
	public static boolean startNewLocalRserve() {
	...
	public static boolean isRserveRunning() {
		return isRserveRunning("localhost", 6311);
	}
	
	/**
	 * check whether Rserve is running on a specific machine and port
	 * @param machine
	 * @param port
	 * @return
	 */
	public static boolean isRserveRunning(String machine, Integer port) {
		try {
			RConnection c = new RConnection(machine, port);
	...
	public static void main(String[] args) {
		System.out.println("result="+startNewLocalRserve());
		try {
			RConnection c=new RConnection("localhost", _port);
			c.shutdown();
		} catch (Exception x) {};
