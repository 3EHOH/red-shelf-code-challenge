...
select 'Step: SQL-costRollups-split.sql' as curstep, current_timestamp as datetime;
\set AUTOCOMMIT on

/* ScriptName:costRollups-split.sql    */
...
/* reset tmp_sub_assocation for split costsÉ */
...
e.master_episode_id,
SUM(CASE
	WHEN soc.association_count>0 THEN (mel.cost/soc.association_count) ELSE (mel.cost)
END) AS cost,
SUM(CASE
	WHEN soc.association_count>0 THEN
	CASE
		WHEN (soc.association_type='T' OR soc.association_type='Typical') THEN 
		(mel.cost_t/soc.association_count + mel.cost_tc/soc.association_count) 
		ELSE 0
	END ELSE
	CASE
		WHEN (soc.association_type='T' OR soc.association_type='Typical') THEN 
		(mel.cost_t + mel.cost_tc) 
		ELSE 0
	END 
END) AS cost_t,
0 AS cost_tc,
SUM(CASE
	...
	CASE
		WHEN (soc.association_type='T' OR soc.association_type='Typical') THEN 
		(mel.cost_c/soc.association_count) 
...
SUM(CASE
	...
	CASE
		WHEN (soc.association_type='T' OR soc.association_type='Typical') THEN 
		(mel.cost_t/soc.association_count) 
		ELSE 0
	END ELSE
	CASE
		WHEN (soc.association_type='T' OR soc.association_type='Typical') THEN 
