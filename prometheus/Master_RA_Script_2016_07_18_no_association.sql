...
#############################################
# excludes applying the coverage gap filter #
#         actually created 7-08             #
#############################################
...
WHERE filter_id=1 and left(master_episode_id,2) in ("EA","EP","EX")
and (age_limit_lower + age_limit_upper + episode_cost_lower + episode_cost_upper + episode_complete + drg + proc_ep_orphan + proc_ep_orph_triggered)>=1
...
(SELECT *
FROM  filtered_episodes
WHERE filter_id=1 and left(master_episode_id,2) in ("EC")
and (age_limit_lower + age_limit_upper + episode_cost_upper + episode_complete + drg + proc_ep_orphan + proc_ep_orph_triggered)>=1
AND ra_episode_data.epi_id=filtered_episodes.master_episode_id);
