...
select 'Step: SQL-costRollups-unsplit-ann.sql' as curstep, current_timestamp as datetime;
\set AUTOCOMMIT on

/* ScriptName:costRollups-unsplit-ann.sql    */
/* Last edited: Jean W 05-4-2017, clean up direct, no cast */

drop table if exists tmp_member_purge;

CREATE TABLE tmp_member_purge AS
SELECT DISTINCT member_id FROM episode WHERE episode_begin_date>episode_end_date;
