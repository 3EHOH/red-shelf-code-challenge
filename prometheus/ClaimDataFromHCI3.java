...
public class ClaimDataFromHCI3 extends ClaimDataAbstract implements ClaimDataInterface {
	 ...
	 * @param errMgr
	 */
	public ClaimDataFromHCI3(ErrorManager errMgr) {
		this.errMgr = errMgr;
	...
	protected boolean doMap(int col_ix, String col_value) {
		...
		if (col_clue == null)
				...
				if (!col_value.isEmpty())
					try {
						if (col_value.contains(".")) {
							svcLine.setQuantity(new Double(Double.parseDouble(col_value)));
						}
						else
							svcLine.setQuantity(new Integer(Integer.parseInt(col_value)));
					}
			...
			case "CapitatedFlag":
				...
				break;
			case "IP_FACILITY_CODE":
			case "facilitytypecode":
	...
	private void setAttributionProvider(String prov_id) {
		if(svcLine.getClaim_line_type_code().equals("PB") )
			if (svcLine.getPhysician_id() == null)
				svcLine.setPhysician_id(prov_id);
		else {
			if (svcLine.getFacility_id() == null)
	...
	*/
	...
	public static void main(String[] args) {
		bDebug = true;
		ClaimDataFromHCI3 instance = new ClaimDataFromHCI3();
		try {
			//instance.addSourceFile(STAY_FILE);
			instance.addSourceFile(CLAIM_FILE);
	...
	//public static final String STAY_FILE = "C:/input/KPMG/doh_to_hci3_20150215/OneBadClaim.csv";
	//public static final String PROF_FILE = "C:\\input\\Verisk\\prof_extract.txt";
	public static final String CLAIM_FILE = "C:\\input\\Driscoll\\DRISCOLL_MEDICAL_CLAIMS_sample.txt";
	
	private static boolean bDebug = false;
	
	private static org.apache.log4j.Logger log = Logger.getLogger(ClaimDataFromHCI3.class);
