...
public class AllDataAPCD extends AllDataAbstract implements AllDataInterface {

	public AllDataAPCD(HashMap<String, String> parameters) {
	...
	public List<ClaimServLine> getAllServiceClaims() {
		...
		try {
			if (cdIf == null) {
				if (errMgr == null)
					cdIf = new ClaimDataFromAPCD(this);
				else
					cdIf = new ClaimDataFromAPCD(this, errMgr);
	...
	public Map<String, List<Enrollment>> getAllEnrollments() {
		...
		try {
			if (enIf == null) {
				if (errMgr == null)
					enIf = new EnrollmentDataFromAPCD();
				else
					enIf = new EnrollmentDataFromAPCD(errMgr);
	...
	public Map<String, PlanMember> getAllPlanMembers() {
		...
		try {
			if (pmIf == null) {
				if (errMgr == null)
					pmIf = new PlanMemberDataFromAPCD();
				else
					pmIf = new PlanMemberDataFromAPCD(errMgr);
	...
	public List<ClaimRx> getAllRxClaims() {
		...
		try {
			if(rxIf == null) {
				if (errMgr == null)
					rxIf = new ClaimRxDataFromAPCD();
				else
					rxIf = new ClaimRxDataFromAPCD(errMgr);
	...
	public Map<String, Provider> getAllProviders() {
		...
		try {
			if (pvIf == null) {
				if (errMgr == null)
					pvIf = new ProviderDataFromAPCD();
				else
					pvIf = new ProviderDataFromAPCD(errMgr);
	...
	private static org.apache.log4j.Logger log = Logger.getLogger(AllDataAPCD.class);
